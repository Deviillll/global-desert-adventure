...
const BuggyTours: React.FC<BuggyToursProps> = ({
  ...
  return (
    <div
      className="h-[70vh] lg:h-[80vh] my-8 w-screen bg-no-repeat  bg-cover bg-center relative lg:bg-[length:140%] flex items-center justify-center lg:justify-end "
      style={{
        backgroundImage:
          `url(${imageUrl})`,
    ...
    >
      ...
      <Card className={`glass backdrop-blur-xs w-[90%] h-[80%]  lg:w-[570px] lg:h-[70vh] relative z-20   px-5  lg:right-[13%]  text-white rounded-3xl`}>
        <CardContent className="">
            <CardTitle className="uppercase text-2xl md:text-4xl lg:text-5xl w-[80%] lg:w-[85%]">{title}<span className="text-orange-400">{colorTitle}</span></CardTitle>
